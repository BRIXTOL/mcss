...
var require$$0$1 = require('escape-string-regexp');
var require$$1 = require('object-assign');
var require$$2 = require('readable-stream/transform');
...
var escapeRegExp = require$$0__default$1['default'];
var objectAssign = require$$1__default['default'];
var Transform = require$$2__default['default'];

var replacestream = function ReplaceStream(search, replace, options) {
  var tail = '';
  var totalMatches = 0;
...
var IdGenerator$1 = function IdGenerator(alphabet) {
  var options = {
...
var idGenerator = IdGenerator$1;

var replaceStream = replacestream;
...
var lib = CssShortener;

/**
 * Console Instance - Used for stdout and stderr
 */
const { log, error, warn } = new console.Console({
...
const config = {
    ...
    options: {
        ...
        sourcemap: true,
        obfuscate: false,
        cacheDir: require$$0.resolve('node_modules/.cache/mcss/.cssmap'),
        typesDir: require$$0.resolve('node_modules/@brixtol/mcss/package/types/classes.d.ts'),
        options: {}
    }
};
 ...
 */
const createCache = (path) => {
    if (!fsExtra.existsSync(path)) {
        if (!fsExtra.existsSync(require$$0.resolve('node_modules/.cache'))) {
            fsExtra.mkdirSync(require$$0.resolve('node_modules/.cache'));
        }
        else {
            fsExtra.mkdirSync(require$$0.resolve('node_modules/.cache/mcss'));
        }
        fsExtra.createFileSync(require$$0.resolve(path));
    }
};
/**
 * Generate Typings
 ...
 */
const writeTypes = async (path) => {
    let types = 'export type ClassNames = Array<';
    const keys = Object.keys(cache.mappings);
    for (const id of keys)
        types += '| "' + id + '"';
    try {
        await fsExtra.writeFile(path, types + '\n>;');
    }
    catch (e) {
        throw error(e);
    }
 ...
 */
const writeMaps = async (config) => {
    try {
        await fsExtra.writeFile(config.cacheDir, JSON.stringify(cache.mappings, null, 2));
    }
    catch (e) {
        throw error(e);
    }
 ...
 */
const generateMaps = (config) => {
    const obfuscate = new lib(config.options);
    return async (css) => {
...
function plugin(provided, defaults) {
    config.available.mcss = true;
    for (const p in provided) {
        if (!(p in config.options))
            throw new Error('No such option ' + p);
...
function postcss(options) {
    if (!config.available.mcss)
        throw new Error('mcss: Missing mcss() output plugin');
    if (!config.available.postcss)
        config.available.postcss = true;
    ...
    return {
        ...
        async Once(root, { result }) {
            ...
            result.root = PostCSS__default['default'].parse(min);
            log(source `{magentaBright mcss {bold generated}}`);
        }
    };
}
    ...
    return {
        ...
        buildStart() {
            if (!config.available.postcss) {
                ...
                log(source `{magentaBright mcss {bold Generating class maps... }}`);
                log(source `{magentaBright mcss {dim Rebuild will execute after generation}`);
            }
        },
        transform(code, id) {
            if (!filter(id))
                return;
            const str = new MagicString__default['default'](code);
            const { warn } = this;
                ...
                enter(node) {
                    if (node.type === 'CallExpression' && node.callee.type === 'MemberExpression') {
                        ...
                        if (object.object?.name === 'm' && object.property?.name === 'css') {
                            let selector = property?.name === 'div'
                                ? ''
                                : property.name;
                            ...
                            str.overwrite(node.start, node.end, '"' + selector + '"');
                            this.remove();
                        }
                    }
                }
